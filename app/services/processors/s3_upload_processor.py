...
from app.services.processors.pipeline import MetricsCollector
...
class S3UploadProcessor(BaseProcessor):
    ...
    async def process(self, input_data: Any, **kwargs) -> str:
        ...
        metric = self._start_processing(ProcessingStage.S3_UPLOAD)
        ...
        if not video_path or not video_id:
            self._end_processing(metric, success=False, error_message="Missing video_path or video_id")
            self.logger.error("Missing video_path or video_id for S3 upload")
            raise UploadError("Missing video_path or video_id for S3 upload", video_id=video_id)
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(None, upload_to_s3)
            ...
            self.logger.error(f"S3 upload failed for {video_path}: {e}", exc_info=True)
            raise UploadError(f"S3 upload failed: {e}", video_id=video_id) from e
