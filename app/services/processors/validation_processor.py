...
from app.services.processors.base_processor import (
    Validator, ValidationResult, ProcessingStage, MetricsCollector
)
...
class VideoRequestValidator(Validator):
    ...
    def __init__(self, metrics_collector: Optional[MetricsCollector] = None):
        ...
        self.optional_segment_fields = [
            "image", "video", "voice_over", "background_music", 
    ...
    def validate(self, data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            self._end_processing(metric, success=result.is_valid, 
    ...
    def _validate_segments(self, segments: List[Dict], result: ValidationResult):
        ...
        for i, segment in enumerate(segments):
            ...
            if "transition_in" in segment:
                self._validate_transition(segment["transition_in"], f"segment {i} transition_in", result)
            
            if "transition_out" in segment:
                self._validate_transition(segment["transition_out"], f"segment {i} transition_out", result)
    
    def _validate_segment_assets(self, segment: Dict, segment_index: int, result: ValidationResult):
        ...
        for asset_type in self.supported_asset_types:
            if asset_type in segment:
                ...
                if not isinstance(asset, dict):
                    result.add_error(f"Segment {segment_index} '{asset_type}' must be a dictionary")
                    continue
                
                # Check for URL or local_path
                if "url" not in asset and "local_path" not in asset:
                    result.add_error(f"Segment {segment_index} '{asset_type}' must have 'url' or 'local_path'")
        ...
        if not has_media:
            result.add_error(f"Segment {segment_index} must have at least one media asset (image or video)")
    
    def _validate_text_overlay(self, text_overlay: Any, segment_index: int, result: ValidationResult):
        ...
        if isinstance(text_overlay, dict):
            ...
            result.add_error(f"Segment {segment_index} 'text_overlay' must be a dict or list")
            return
        
        for i, overlay in enumerate(text_overlays):
            if not isinstance(overlay, dict):
                result.add_error(f"Segment {segment_index} text_overlay {i} must be a dictionary")
                continue
            
            if "text" not in overlay:
                result.add_error(f"Segment {segment_index} text_overlay {i} missing required 'text' field")
            
            # Validate timing
            if "start_time" in overlay:
                if not isinstance(overlay["start_time"], (int, float)) or overlay["start_time"] < 0:
                    result.add_error(f"Segment {segment_index} text_overlay {i} 'start_time' must be a non-negative number")
            
            if "end_time" in overlay:
                if not isinstance(overlay["end_time"], (int, float)) or overlay["end_time"] < 0:
                    result.add_error(f"Segment {segment_index} text_overlay {i} 'end_time' must be a non-negative number")
            
            # Validate start_time < end_time
            if "start_time" in overlay and "end_time" in overlay:
                if overlay["start_time"] >= overlay["end_time"]:
                    result.add_error(f"Segment {segment_index} text_overlay {i} 'start_time' must be less than 'end_time'")
    
    def _validate_transition(self, transition: Dict, location: str, result: ValidationResult):
        ...
        if "type" in transition:
            if transition["type"] not in self.supported_transition_types:
                result.add_error(f"{location} type '{transition['type']}' not supported. "
                               f"Supported types: {', '.join(self.supported_transition_types)}")
        
        if "duration" in transition:
            if not isinstance(transition["duration"], (int, float)) or transition["duration"] <= 0:
    ...
    def _validate_global_settings(self, data: Dict[str, Any], result: ValidationResult):
        ...
        if "background_music" in data:
            ...
            if not isinstance(bg_music, dict):
                ...
                result.add_error("Global 'background_music' must have 'url' or 'local_path'")
        ...
        if "output_settings" in data:
            ...
            if not isinstance(output_settings, dict):
                ...
                if "fps" in output_settings:
                    if not isinstance(output_settings["fps"], int) or output_settings["fps"] <= 0:
                        result.add_error("'output_settings.fps' must be a positive integer")
                ...
                if "resolution" in output_settings:
                    ...
                    if not isinstance(resolution, str) or "," not in resolution:
                        result.add_error("'output_settings.resolution' must be a string in format 'width,height'")
        ...
        for i, transition in enumerate(transitions):
            ...
            if "from_segment" in transition:
                if not isinstance(transition["from_segment"], (str, int)):
                    result.add_error(f"Transition {i} 'from_segment' must be a string or integer")
            
            if "to_segment" in transition:
                if not isinstance(transition["to_segment"], (str, int)):
                    result.add_error(f"Transition {i} 'to_segment' must be a string or integer")


# Export the main validator
def create_video_request_validator(metrics_collector: Optional[MetricsCollector] = None) -> VideoRequestValidator:
